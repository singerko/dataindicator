// OverlayView.swift
// SwiftUI overlay indikátor pre iOS (tenký prúžok navrchu).

import SwiftUI

/// Overlay prúžok, ktorý mení farbu podľa typu siete.
struct OverlayView: View {
    @EnvironmentObject var networkMonitor: NetworkMonitor
    @AppStorage("indicatorHeight") private var indicatorHeight: Double = 4.0
    @AppStorage("indicatorWidthPercent") private var indicatorWidthPercent: Double = 100.0
    @AppStorage("alignment") private var alignment: String = "center"
    @AppStorage("wifiColor") private var wifiColorHex: String = "#4CAF50"
    @AppStorage("mobileColor") private var mobileColorHex: String = "#F44336"
    @AppStorage("noInternetColor") private var noInternetColorHex: String = "#9E9E9E"
    
    /// UI overlay prúžku.
    var body: some View {
        GeometryReader { geometry in
            VStack {
                Rectangle()
                    .fill(currentColor)
                    .frame(
                        width: geometry.size.width * (indicatorWidthPercent / 100.0),
                        height: indicatorHeight
                    )
                    .animation(.easeInOut(duration: 0.3), value: networkMonitor.networkType)
                
                Spacer()
            }
            .frame(maxWidth: .infinity, maxHeight: .infinity, alignment: alignmentValue)
        }
        .allowsHitTesting(false) // Povoliť interakciu s aplikáciami pod overlay
        .ignoresSafeArea(.all)
    }
    
    /// Farba podľa aktuálneho typu siete.
    private var currentColor: Color {
        switch networkMonitor.networkType {
        case .wifi:
            return Color(hex: wifiColorHex) ?? .green
        case .cellular:
            return Color(hex: mobileColorHex) ?? .red
        case .none:
            return Color(hex: noInternetColorHex) ?? .gray
        }
    }
    
    /// Zarovnanie prúžku podľa nastavení.
    private var alignmentValue: Alignment {
        switch alignment {
        case "left":
            return .topLeading
        case "right":
            return .topTrailing
        default:
            return .top
        }
    }
}

/// Pomocný inicializátor farby z HEX reťazca.
extension Color {
    /// Vytvorí Color z HEX reťazca (`#RRGGBB` alebo `#AARRGGBB`).
    init?(hex: String) {
        let hex = hex.trimmingCharacters(in: CharacterSet.alphanumerics.inverted)
        var int: UInt64 = 0
        Scanner(string: hex).scanHexInt64(&int)
        let a, r, g, b: UInt64
        switch hex.count {
        case 3: // RGB (12-bit)
            (a, r, g, b) = (255, (int >> 8) * 17, (int >> 4 & 0xF) * 17, (int & 0xF) * 17)
        case 6: // RGB (24-bit)
            (a, r, g, b) = (255, int >> 16, int >> 8 & 0xFF, int & 0xFF)
        case 8: // ARGB (32-bit)
            (a, r, g, b) = (int >> 24, int >> 16 & 0xFF, int >> 8 & 0xFF, int & 0xFF)
        default:
            return nil
        }

        self.init(
            .sRGB,
            red: Double(r) / 255,
            green: Double(g) / 255,
            blue:  Double(b) / 255,
            opacity: Double(a) / 255
        )
    }
}

#Preview {
    OverlayView()
        .environmentObject(NetworkMonitor())
}
